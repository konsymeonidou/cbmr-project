...
def create_HealthMetrics(request: Request, HealthMetrics: HealthMetrics = Body(...)):
    ...
    created_HealthMetrics = get_collection_metrics(request).find_one(
        {"_id": new_HealthMetrics.inserted_id}
    )
...
def find_HealthMetrics(request: Request, id: str):
    ...
    raise HTTPException(
        ...
        detail=f"HealthMetrics with id {id} not found!",
    )


def delete_HealthMetrics(request: Request, id: str):
    deleted_HealthMetrics = get_collection_metrics(request).delete_one(
        {"_id": ObjectId(id)}
    )
