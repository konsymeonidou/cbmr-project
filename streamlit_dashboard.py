    ...
    if data:
        ...
        if not df.empty:
            ...
            if "age" in df.columns:
                fig = px.histogram(df, x="age", title="Age Distribution of Candidates")
            ...
            if "email" in df.columns:
                ...
                fig = px.pie(email_status_counts, names="status", values="count", title="Email Availability")
                fig.update_layout(height=400, margin=dict(l=20, r=20, t=40, b=20))
                st.plotly_chart(fig, use_container_width=True)
        else:
    ...
    if data:
        ...
        if not df.empty:
            ...
            if "date" in df.columns and "daily_metrics.steps" in df.columns:
                ...
                fig = px.line(df, x="date", y="daily_metrics.steps", title="Steps Over Time")
                fig.update_layout(height=400, margin=dict(l=20, r=20, t=40, b=20))
                st.plotly_chart(fig, use_container_width=True)
            ...
            if "daily_metrics.weight" in df.columns:
                ...
                fig = px.pie(weight_distribution, names="weight_range", values="count", title="Weight Distribution")
                fig.update_layout(height=400, margin=dict(l=20, r=20, t=40, b=20))
                st.plotly_chart(fig, use_container_width=True)
        else:
